...
import { SPONSOR_ROSTER, buildLogos, renderModule } from '../lib/sponsorLogos.gen';
...
const logos = buildLogos(SPONSOR_ROSTER, hashFn);
writeFileSync(resolve(process.cwd(), 'components', 'sponsorLogos.ts'), renderModule(logos));
console.log(`generated components/sponsorLogos.ts with content hashes for ${logos.length} logos`);
